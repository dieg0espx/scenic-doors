...
import { updateLeadSharedWith } from "@/lib/actions/leads";
...
interface LeadShareCardProps {
  leadId: string;
  salesReps: SalesRep[];
  initialSharedWith: string[];
}

export default function LeadShareCard({
  leadId,
  salesReps,
  initialSharedWith,
}: LeadShareCardProps) {
  ...
  function toggle(userId: string) {
    ...
    startTransition(async () => {
      try {
        await updateLeadSharedWith(leadId, next);
      } catch {
        // Revert on error
