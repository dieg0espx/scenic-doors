...
export default function Testimonials() {
  return (
    <section className="py-24 md:py-32 bg-ocean-900 relative overflow-hidden">
      {/* Background accent */}
      <div className="absolute top-0 right-0 w-1/2 h-full">
        <div className="absolute inset-0 bg-gradient-to-l from-wood-900/20 to-transparent" />
      </div>
      ...
      <div className="section-container relative">
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-6">
            <div className="w-12 h-[1px] bg-wood-500" />
            <Quote className="w-6 h-6 text-wood-400" />
            <div className="w-12 h-[1px] bg-wood-500" />
          </div>
          <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl text-white mb-4">
            Trusted by Southern California's
            <span className="block text-wood-400">Most Discerning Homeowners</span>
          </h2>
        ...
        <div className="space-y-8">
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div className={`bg-ocean-800/50 backdrop-blur-sm p-8 md:p-10 max-w-2xl relative ${index % 2 === 1 ? "md:mr-12" : "md:ml-12"}`}>
                {/* Quote mark */}
                <span className="absolute -top-4 -left-2 font-heading text-7xl text-wood-500/30">
                  "
                </span>
                ...
                <div className="flex gap-1 mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 fill-wood-400 text-wood-400" />
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-wood-400 text-sm">{testimonial.project}</p>
                  </div>
                </div>

                {/* Accent line */}
                <div className="absolute bottom-0 left-0 w-24 h-1 bg-wood-500" />
              </div>
            </motion.div>
          ))}
        </div>
